/**
 * FileReader — dependon infra.
 *
 * Safe file reader with size limits, streaming for large files, and timeout
 * protection. Shared by languages and resolution layers.
 *
...
export class FileReader {
   ...
   *
   * @throws DependonError with FILE_TOO_LARGE if file exceeds maxSize
   * @throws DependonError with TIMEOUT if reading takes too long
   */
  async readFile(filePath: string, options?: FileReadOptions): Promise<string> {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(new DependonError(
          `File read timed out after ${timeout}ms: ${filePath}`,
          DependonErrorCode.TIMEOUT,
          { filePath }
        ));
      }, timeout);
    });
    ...
    try {
      ...
      if (stats.size > maxSize) {
        throw new DependonError(
          ...
          DependonErrorCode.FILE_TOO_LARGE,
          { filePath }
        );
      }
  ...
  private async readFileStreaming(filePath: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      stream.on('error', (error) => {
        reject(DependonError.fromError(error, filePath));
      });
    });
  }
  ...
  async readFileIfExists(filePath: string): Promise<string | null> {
    try {
      ...
      if (error instanceof DependonError && error.code === DependonErrorCode.FILE_NOT_FOUND) {
        return null;
      }
      throw error;
    }
