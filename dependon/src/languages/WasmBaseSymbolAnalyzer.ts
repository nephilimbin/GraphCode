...
import { ISymbolAnalyzer, SymbolDependency, SymbolInfo } from '../domain/Symbol';
...
/**
 * WasmBaseSymbolAnalyzer — dependon languages.
 *
 ...
 *
 * TypeScript 的分析器使用 ts-morph(另一套 AST 栈),不继承本类。
 *
  ...
  protected addCallDependencyIfAny(
    ...
    if (target.moduleQualifier && importMap?.has(target.moduleQualifier)) {
      ...
      if (moduleSpecifier !== undefined) {
        // targetFilePath holds the module specifier; resolved to an absolute
        // path later by the symbol graph reader.
        dependencies.push({
          sourceSymbolId: scope,
