...
import type { ILanguageAnalyzer } from "../domain/Symbol";
...
export class JavaParser implements ILanguageAnalyzer {
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    this.initPromise = (async () => {
      try {
        ...
        const javaWasmPath = resolveWasmFile("tree-sitter-java.wasm", this.extensionPath);
  ...
  async parseImports(filePath: string): Promise<Dependency[]> {
    ...
    try {
      ...
      throw DependonError.fromError(error, actualPath);
    }
