 ...
 */
import { FileReverseIndex } from './FileReverseIndex';
import { SymbolReverseIndex } from './SymbolReverseIndex';
import type { Dependency, FileHash } from '../domain/Dependency';
import type { SymbolDependency } from '../domain/Symbol';

/**
 * Encapsulates reverse index lifecycle and operations for Spider/MCP.
 */
export class ReverseIndexStore {
  private reverseIndex: FileReverseIndex | null = null;
  ...
  enable(serializedData?: string): boolean {
    let restored = false;
    this.reverseIndex ??= new FileReverseIndex(this.rootDir);
    this.symbolReverseIndex ??= new SymbolReverseIndex(this.rootDir);

    if (serializedData) {
      try {
        const data = JSON.parse(serializedData);
        const restoredIndex = FileReverseIndex.deserialize(data, this.rootDir);
        if (restoredIndex) {
