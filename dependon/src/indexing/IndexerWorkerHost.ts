...
// ESM does not provide `__dirname`; derive the current module directory from import.meta.url.
...
export class IndexerWorkerHost {
  ...
  constructor(workerPath?: string) {
    // Default to the worker file under dist/workers/ (esbuild bundle outfile, lowercase).
    // The compiled IndexerWorker.ts is emitted as `indexerWorker.js` (lowercase) by esbuild;
    // resolve relative to the current module directory so it matches the ESM bundle layout.
    this.workerPath = workerPath ?? path.join(currentDir, 'workers', 'indexerWorker.cjs');
  }
