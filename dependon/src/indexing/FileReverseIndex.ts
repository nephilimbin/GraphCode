...
export class FileReverseIndex {
  ...
  static deserialize(
    ...
    const index = new FileReverseIndex(rootDir);
  ...
  async validateIndex(staleThreshold: number = 0.2): Promise<{
    ...
    for (const filePath of this.fileHashes.keys()) {
      const currentHash = await FileReverseIndex.getFileHashFromDisk(filePath);
