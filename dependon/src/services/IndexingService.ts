...
import { YIELD_INTERVAL_MS } from '../core/EventLoopYield';
import { DependencyAnalyzer } from './DependencyAnalyzer';
import { Cancellation } from './Cancellation';
import { WorkerManager } from './WorkerManager';

const log = getLogger('IndexingService');
...
export class IndexingService {
  ...
  async buildFullIndex(
    ...
    try {
      ...
      const dependonError = DependonError.fromError(error);
      this.indexerStatus.setError(dependonError.toUserMessage());
      log.error('Indexing failed:', dependonError.toUserMessage(), dependonError.code);
      throw dependonError;
    }
  }
